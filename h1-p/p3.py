...
numeric_tag = "Numeric"
all_capitals_tag = "All_capitals"
last_capitals_tag = "Last_capitals"
	...
	while l:
		...
		if line:
			...
			if word in rare_words:
				p = re.search(r'.*[0-9]+', word)
				if p:
					output_file.write("%s %s\n" % (numeric_tag, tag))
				else:
					if word.isupper():
						output_file.write("%s %s\n" % (all_capitals_tag, tag))
					elif word.isupper() == False and word[len(word) - 1] in uppercase_letters:
						output_file.write("%s %s\n" % (last_capitals_tag, tag))
					else:
						output_file.write("%s %s\n" % (rare_word_tag, tag))
			else:
				output_file.write("%s %s\n" % (word, tag))
		...
		for sentence in sentence_iterator:
			...
			for i in xrange(sentence_len):
				if current_sentence[i] not in self.words:
					p = re.search(r'.*[0-9]+', current_sentence[i])
					if p:
						replace_sentence[i] = numeric_tag
					else:
						if current_sentence[i] .isupper():
							replace_sentence[i] = all_capitals_tag
						elif current_sentence[i] .isupper() == False and current_sentence[i][len(current_sentence[i]) - 1] in uppercase_letters:
							replace_sentence[i] = last_capitals_tag
						else:
...
def main():
	...
	train_replace_filename = "gene.p3.train"
	test_data_filename = "gene.test"
	test_output_filename = "gene_test.p3.out"
